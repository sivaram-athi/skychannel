...
const EditChannel = ({ id, setEdit, onRoomAdded }) => {
  // const handleCancel = () => {
  //   setEdit(0);
  // };
  ...
  useEffect(() => {
    $.ajax({
      url: "https://beedesk.skyhms.in/skychnl/getchannelbyid",
      type: "POST",
      data: { id: id },
      dataType: "json",
      success: function (response) {
        ...
        setFormData({
          ...
          chnltoken: response.chnltoken,
          category: response.otaid,
  ...
  const handleSave = () => {
    if (validateForm()) {
      ...
      $.ajax({
        url: "https://beedesk.skyhms.in/skychnl/updatechannel",
        type: "POST",
